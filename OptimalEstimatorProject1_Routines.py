...
def WeightedObsTransfer(ID,Hours,Exp,P,OptionToSet): #Determining Weight Matrix 
    ...
    if P[1] == 0: #Est of Var Unit Weight is Taken as 1 and divided by SD to give Weight Mat
            ...
            EstVarOfUnitWeight = EstVarUnitWeight(ID,Ind,Exp,P,OptionToSet,Pos_ERR,0)
            ...
            EstVarOfUnitWeight = EstVarUnitWeight(ID,Ind,Exp,P,OptionToSet,Pos_ERR,1)
            P_Posx, P_posy, P_posz = EstVarOfUnitWeight[0]/Pos_ERR[:,0],EstVarOfUnitWeight[1]/Pos_ERR[:,1],EstVarOfUnitWeight[2]/Pos_ERR[:,2] 
            ...
            Pos_ERR = np.stack([P_Posx,P_Posx,P_Posx],axis=1)          
            VarEst3 = EstVarUnitWeight(ID,Ind,Pos_ERR,P,OptionToSet,Pos_ERR,0)[2]         
...
def EstVarUnitWeight(ID,Ind,Exp,P,OptionToSet,Pos_ERR,Weight_Itt): #Estimating Variance of Unit Weight
    if Weight_Itt == 1:
        P = [0,0]
        [Fit,Type,Omega,Eq0,Eq1,Eq2,Eq0_Residual,Eq1_Residaul,Eq2_Residual] = LSFittoData(Ind,Exp,P,ID,OptionToSet)
        Eqs = [Eq0_Residual,Eq1_Residaul,Eq2_Residual] #Observation Errors (derived from LS residaul)
        VarUnitW_hat = []   
